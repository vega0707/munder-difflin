 ...
 *  shows those engines too, as disabled workers-only rows — every preset that
 *  can drain hive mail is selectable, and every other preset except `custom`
 *  (bring-your-own command, not an engine) is in the disabled group.
 *
 *  God-eligible is `canReceiveInbox`, NOT `modelProvidersForAgent(true)`: the
 *  latter also demands a model list, and the built-in engine has none
 *  (`supportsModel: false`) even though it ships as DEFAULT_AGENT_PROVIDER and
 *  runs the floor on its own. Keying the selectable group off the model picker
 *  filed the default engine under the disabled rows. */
export function onboardingEngineChoices(): {
  eligible: AgentProviderPreset[];
  workersOnly: AgentProviderPreset[];
} {
  const eligible = AGENT_PROVIDER_PRESETS.filter((preset) => preset.canReceiveInbox);
