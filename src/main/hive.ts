...
export class HiveManager {
  ...
  private buildDefaultMcpServers(
    ...
    // Resolved on first use, not up front: the socket path reads the app config
    // through Electron's `app.getPath`, which a plain-node test run has no `app`
    // for. Only a server that actually spawns with a `<sock>` placeholder needs it.
    let sockPath: string | undefined;
    const resolveSockPath = (): string => (sockPath ??= automationSocketPath());
    ...
    for (const e of MCP_CATALOG) {
      ...
      if (e.spec.env) {
        ...
        for (const [key, val] of Object.entries(e.spec.env)) {
          env[key] = val === '<sock>' ? resolveSockPath() : val;
        }
  ...
  private identityText(meta: AgentMeta): string {
    ...
    return [
      ...
      meta.isGod
        ? `- Coordinator toolkit: skills \`${COORDINATOR_SKILLS.join('`, `')}\`; safe MCP \`${COORDINATOR_MCP_SAFE.join('`, `')}\`; consent-gated search \`${COORDINATOR_MCP_CONSENT.join('`, `')}\` (Settings → MCP).`
        : '',
      ''
    ].filter(Boolean).join('\n');
  }
